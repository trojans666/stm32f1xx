...
static char tmp_buf[MENU_CONSOLE_BUFFER]; 	/* copy of console I/O buffer	*/
#if 1
static int do_dummy_start(cmd_tbl_t *cmdtp,int flag,int argc,char *const argv[])
{
    return 0;
}

CMD_COMPLETE("1.2.start",start,0,1,do_dummy_start,"cmd start postion","",NULL);

static int do_dummy_end(cmd_tbl_t *cmdtp,int flag,int argc,char *const argv[])
{
    return 0;
}

CMD_COMPLETE("2.2.end",end,0,1,do_dummy_end,"cmd end postion","",NULL);
#endif
/* _cmd_start _cmd_end */

cmd_tbl_t *find_cmd(const char *cmd)
{
    int len = &_cmd_end - &_cmd_start;
    console_printf("find len = %d \n",len);
    return find_cmd_tbl(cmd,&_cmd_start,len);
}
...
int cmd_usage(const cmd_tbl_t *cmdtp)
{
	...
	if (!cmdtp->help) {
		console_puts ("- No additional help available.\n");
		return 1;
	}
...
static int complete_cmdv(int argc, char * const argv[], char last_char, int maxv, char *cmdv[])
{
	...
	if (argc == 0) {
		...
		for (cmdtp = &_cmd_start; cmdtp != &_cmd_end; cmdtp++) {
			if (n_found >= maxv - 2) {
				cmdv[n_found++] = "...";
				break;
			}
...
int cmd_auto_complete(const char *const prompt, char *buf, int *np, int *colp)
{
	...
	if (strcmp(prompt, MENU_PROMPT_NAME) != 0)
