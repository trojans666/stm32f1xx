...
typedef struct serial_
{
    ...
    void (*Initialize)(struct serial_ *serial);
    void (*SetParams)(unsigned char No,serial_params_t *params);
	unsigned int (*GetMsg)(struct serial_ *serial, void *pRecvBuf, unsigned int len);	//??n???
	void (*SendMsg)(struct serial_ *serial, void *pSendBuf, unsigned int len);
	void (*RtCtrl)(unsigned char No, unsigned char rtStatus);
	void (*Interrupt)(struct serial_ *serial);
